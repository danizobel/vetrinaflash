...
function FAQItem({ faq, index }: { faq: (typeof faqs)[0]; index: number }) {
  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, delay: (index % 5) * 0.06 }} className={`relative border rounded-2xl overflow-hidden transition-all duration-300 ${open ? "border-[#7CFF00]/25 bg-[#7CFF00]/[0.03]" : "border-white/8 bg-white/[0.02] hover:border-white/12"}`}>
      ...
      <button onClick={() => setOpen(!open)} className="relative w-full flex items-center justify-between gap-4 px-6 py-5 text-left z-10">
        <span className="text-white font-medium text-base">{faq.q}</span>
        <motion.div animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.2 }} className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center transition-all ${open ? "bg-[#7CFF00]/20" : "bg-white/8"}`} style={{ filter: open ? "drop-shadow(0 0 8px rgba(124,255,0,0.6))" : "none" }}>
          <ChevronDown className="w-4 h-4" style={{ color: open ? "#7CFF00" : "rgba(255,255,255,0.5)" }} />
        </motion.div>
      </button>
      <AnimatePresence initial={false}>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ type: "spring", stiffness: 280, damping: 22 }} className="relative z-10">
            <div className="px-6 pb-5"><div className="w-full h-px bg-white/6 mb-4" /><p className="text-white/60 text-sm leading-relaxed">{faq.a}</p></div>
          </motion.div>
        )}
      </AnimatePresence>
    </motion.div>
...
export default function FAQSection() {
  ...
  return (
    <section id="faq" className="relative py-24 md:py-32 overflow-hidden aurora">
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6">
        <div ref={headingRef} className="text-center mb-14">
          ...
          <motion.h2 initial={{ opacity: 0, y: 30 }} animate={headingInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.1 }} className="font-display text-[clamp(1.75rem,8.4vw,2.125rem)] sm:text-5xl md:text-6xl font-extrabold tracking-tight mb-5">Risposte dirette.<br /><span className="neon-text">Nessun giro di parole.</span></motion.h2>
        </div>
        <div className="flex flex-col gap-3">{faqs.map((faq, i) => <FAQItem key={faq.q} faq={faq} index={i} />)}</div>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="mt-10 text-center p-7 rounded-3xl border border-white/8 bg-white/[0.02] glow-pulse animated-border relative overflow-hidden">
          <p className="text-white/60 text-base mb-4 relative z-10">Hai ancora dubbi? Parliamone direttamente.</p>
          <motion.a href={WA_DEFAULT} target="_blank" rel="noopener noreferrer" whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} className="relative z-10 inline-flex items-center gap-2 px-6 py-3 rounded-full border border-[#25D366]/30 bg-[#25D366]/8 text-[#25D366] font-semibold text-sm hover:bg-[#25D366]/15 transition-all duration-200 shimmer"><MessageCircle size={16} />Scrivici su WhatsApp — {PHONE_DISPLAY}</motion.a>
        </motion.div>
      </div>
    </section>
  );
