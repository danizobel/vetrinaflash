    ...
    <section id="problema" className="relative py-24 md:py-32 overflow-hidden scan-lines">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid sm:grid-cols-3 gap-5 mb-14">
          {platforms.map((p) => (
            <motion.div key={p.name} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-10%" }} transition={{ delay: p.delay, duration: 0.6, ease: [0.22, 1, 0.36, 1] }} whileHover={{ y: -6, rotate: [0, -0.5, 0.5, 0], boxShadow: "0 0 40px rgba(255,61,61,0.25)" }} className="animated-border relative p-5 sm:p-7 rounded-3xl border border-red-500/15 bg-gradient-to-b from-red-500/[0.06] to-transparent overflow-hidden group">
              <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-red-500/50 to-transparent" />
              <div className="text-white/45 text-xs sm:text-sm font-semibold uppercase tracking-widest mb-3 sm:mb-4">{p.name}</div>
              <div className="font-display text-5xl sm:text-7xl font-extrabold text-red-400 mb-1 tabular-nums transition-all duration-300 group-hover:text-red-300" style={{ textShadow: "0 0 44px rgba(255,61,61,0.35)" }}><CountUp to={p.percent} suffix="%" duration={1.2 + p.delay} /></div>
              <div className="text-white/30 text-[11px] sm:text-xs uppercase tracking-widest mb-5 sm:mb-6">di commissione per ordine</div>
              <div className="p-3 sm:p-3.5 rounded-xl bg-black/30 border border-white/5 text-xs sm:text-sm leading-relaxed">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-6 mb-12 sm:mb-14 text-center">
          <span className="text-white/40 text-sm sm:text-base line-through decoration-red-400/60">Fino al 35% di commissione</span>
          <motion.span animate={{ x: [0, 8, 0] }} transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }} className="text-[#7CFF00] hidden sm:inline-block"><ArrowRight size={22} /></motion.span>
          <span className="font-display text-lg sm:text-2xl font-bold neon-text">Con VetrinaFlash: 0%. Mai.</span>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-10%" }} transition={{ duration: 0.7 }} animate={{ boxShadow: ["0 0 15px rgba(124,255,0,0.05)", "0 0 40px rgba(124,255,0,0.15)", "0 0 15px rgba(124,255,0,0.05)"] }} className="card-lux glow-pulse rounded-3xl p-6 sm:p-12 text-center" style={{ transition: "box-shadow 3s ease-in-out infinite" }}>
          <p className="text-white/50 text-xs sm:text-base mb-3 sm:mb-4">Con 15 ordini al giorno da €25 medi, con VetrinaFlash risparmieresti <span className="text-white font-semibold">ogni anno</span>:</p>
          <div className="font-display text-4xl sm:text-7xl md:text-8xl font-extrabold neon-text tabular-nums mb-3"><CountUp to={41063} prefix="€" duration={2.2} /></div>
          <p className="text-white/25 text-[11px] sm:text-xs">calcolato su commissione media del 30% · stima su volumi medi di un locale tipo</p>
        </motion.div>
      </div>
    </section>
  );
